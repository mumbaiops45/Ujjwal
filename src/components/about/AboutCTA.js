...
export default function AboutCTA() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      .fromTo(Array.from(pillsRef.current.children),
        { y: 18, opacity: 0 },
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden bg-[var(--primary)] py-28">
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle at 70% 25%, var(--secondary) 0%, transparent 65%)",
        }}
      />
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle at 30% 75%, var(--secondary) 0%, transparent 65%)",
        }}
      />
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-10 text-center">
        ...
        >
          ...
          <span className="text-[var(--secondary)]">Across India</span>
        </h2>

        {/* Sub-copy */}
        <p
          ref={subRef}
          className="text-white/60 leading-relaxed text-sm sm:text-base mb-10 max-w-xl mx-auto"
        >
          From industrial packaging to garment trim manufacturing, Ujjwal Poly Pack
          delivers scalable quality with reliability across every order. Speak with
          our team for a tailored manufacturing solution for your business.
        </p>
        ...
        >
          ...
          onClick={() => window.dispatchEvent(new CustomEvent("open-enquiry-modal"))}
            className="cursor-pointer inline-flex items-center gap-3 px-8 py-4 bg-[var(--secondary)] text-white text-sm font-bold uppercase tracking-wide hover:bg-[var(--secondary)]/90 transition-all duration-300 group w-full sm:w-auto justify-center"
          >
            Request A Quote
